...
# Just run:  python3 k8s-install-upgrade.py
...
def check_dependencies():
    ...
    if errors:
        ...
        for e in errors:
            print(f"  ❌ {e}\n")
        print("  After installing, run:  python3 k8s-install-upgrade.py\n")
        sys.exit(1)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Kubernetes Lifecycle Manager — just run: python3 k8s-install-upgrade.py",
...
def update_inventory(remote_config):
    """
    Write inventory/hosts.ini at RUNTIME only.
    This file is in .gitignore — never committed to git.
    Each run generates a fresh file from wizard input.
    No server IPs are ever stored permanently.
    """
    ...
    inventory_content = f"""# AUTO-GENERATED AT RUNTIME — DO NOT COMMIT
# Generated by k8s-install-upgrade.py
...
"""
    ...
    print(f"  [INFO] Ansible inventory written → inventory/hosts.ini (runtime only)")
...
def run_upgrade_flow(data, mode="local"):
    ...
    from modules.version_manager        import get_upgrade_information, set_remote_config as vm_set_remote
    from modules.validation             import check_cluster_health, set_remote_config as val_set_remote
    ...
    # Pass remote config to all modules that call kubectl
    # Fixes: kubectl not found on Windows when running in remote mode
    # All kubectl commands must run on the SERVER via SSH, not locally
    if mode == "remote" and _remote_config_ref:
        vm_set_remote(_remote_config_ref)
...
def main():
    ...
    if mode == "remote":
        ...
        try:
            ...
            print(f"\n❌ Connection failed: {e}")
            print("\nFor help: python3 k8s-install-upgrade.py --setup\n")
            sys.exit(1)
    ...
    if state in ["NOT_INSTALLED", "INSTALLED_NO_CLUSTER",
        ...
        if success:
            print("\n✔ Cluster installed! Re-run: python3 k8s-install-upgrade.py\n")
        else:
            print("\n❌ Installation failed.\n")
        return
